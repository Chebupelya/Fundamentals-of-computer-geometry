...
public:
	...
	//dc: Это ссылка на контекст устройства(CDC), который представляет собой интерфейс для рисования на устройстве вывода(например, экране или принтере).
	//Radius : Это радиус сферы.
	//PView : Это матрица, представляющая координаты точки наблюдения в сферической системе координат.
	//PSourceLight : Это матрица, представляющая координаты источника света в сферической системе координат.
	//RW : Это прямоугольник, представляющий область в окне, на которой будет производиться рисование.
	//Color : Это цвет освещения в формате COLORREF.Он определяет базовый цвет освещенности.
	//Index : Это индекс, определяющий тип освещения.Если Index == 0, то используется диффузионное освещение.Если Index == 1, то используется зеркальное отражение света.
};
